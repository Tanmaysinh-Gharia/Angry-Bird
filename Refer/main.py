...
static_lines = [pm.Segment(static_body, (0.0, 060.0), (1200.0, 060.0), 0.0), pm.Segment(
    static_body, (0.0, 0.0), (0.0, 600.0), 0.0)]
static_lines1 = [pm.Segment(static_body, (1200.0, 060.0), (1200.0, 800.0), 0.0), pm.Segment(
    static_body, (0.0, 650.0), (1200.0, 650.0), 0.0)]
...
def sling_action():
    ...
    if mouse_distance > rope_lenght:
        ...
        pygame.draw.line(screen, (0, 0, 0), (sling2_x, sling2_y), pu2, 6)
        screen.blit(redbird, pul)
        pygame.draw.line(screen, (0, 0, 0), (sling_x, sling_y), pu2, 6)
    else:
        x_redbird = x_mouse - 20
        y_redbird = y_mouse - 20
        mouse_distance += 10
        pu3 = (uv1*mouse_distance+sling_x, uv2*mouse_distance+sling_y)
        pygame.draw.line(screen, (0, 0, 0), (sling2_x, sling2_y), pu3, 8)
        screen.blit(redbird, (x_redbird, y_redbird))
        pygame.draw.line(screen, (0, 0, 0), (sling_x, sling_y), pu3, 8)
    # Angle of impulse
...
while running:
    ...
    # pygame.draw.circle(screen,(0,0,0),(154,650-156),15)
    # Draw first part of the sling
    # rect = pygame.Rect(50, 0, 70, 220)
    screen.blit(sling_image, (138, 420), (50, 0, 70, 220))
    ...
    if level.number_of_birds > 0:
        for i in range(level.number_of_birds-1):
            x = 100 - (i*35)
            # pygame.draw.circle(screen, BLACK, (100,508), 5, 0)
    ...
    for bird in birds:
        ...
        # print(bird.has_collided)
        if (counter >= 4 and (bird.has_collided == False)):
            bird_path.append(p)
            counter = 0
    ...
    # dt = 1.0/50.0/2.
    # for x in range(2):
    space.step(1/50)  # make two updates per frame for better stability
    ...
    # pygame.draw.rect(screen,(0,0,0),(300,0,500,800))``
    # pygame.draw.circle(screen,BLACK,(950,550),7,0)
    # pygame.draw.polygon(screen,BLACK,((950,550),()),7,0)
    # Pause option
    if game_state == 1:

        pygame.mixer.music.pause()
        img = pygame.image.load("Untitled.png").convert_alpha()
        img = pygame.transform.scale(img, (1200, 650))
        img.set_alpha(50)
        mouse_pressed = False
    ...
    pygame.display.flip()
    clock.tick(50)
    pygame.display.set_caption(" Angry Bird " + "     LEVEL : " + str(level.number + 1) +
